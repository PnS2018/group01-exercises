...
number_batches = num_train_samples/batch_size
concat_train = np.concatenate((train_X,np.expand_dims(train_y, axis=1)),axis=1)

for i in range(num_epochs):
    np.random.shuffle(concat_train)
    train_X_rand = concat_train[:, :-1]
    train_y_rand = concat_train[:, -1:]
    for j in range(number_batches):
        train_function((train_X_rand[j*batch_size:(j+1)*batch_size],train_y_rand[j*batch_size:(j+1)*batch_size]))
